...
import { editColumnTitle } from '../../actions/listActions';
  ...
  columnTitleWrapper = () => {
    if (this.state.isColumnTitleInEdit) {
      return (
        <input
          type={'text'}
          className={classes.title__edit}
          value={this.state.columnTitle}
          onChange={this.onTitleChangeHandler}
          onKeyDown={this.onKeyDownTitleHandler}
        />
      );
    } else {
      return (
        <h3 className={classes.title}>
          ...
          <span
            className={classes.title__editIcon}
            onClick={this.columnTitleEditIconHandler}
          >
            <EditLogo />
          </span>
        </h3>
    ...
    if (this.state.isNewCardTitleInEdit) {
      return (
        <React.Fragment>
          <textarea
            ...
            className={classes.card_title__inedit}
            value={this.state.newCardTitle}
            onChange={this.onNewCardTitleChangeHandler}
            onKeyDown={this.onAddCardKeyDownHandler}
          />
          <button
            className={classes.card_title__addcard}
            onClick={this.onAddCardKeyDownHandler}
            name={'addcard'}
          >
            Add Card
          </button>
          <span
            className={classes.card_title__closeicon}
            onClick={this.onAddCardCloseIconHandler}
          >
            {' '}
            <CloseIcon />
          </span>
      ...
      return (
        <React.Fragment>
          ...
          <p
            onClick={this.onAddCardClickedHandler}
            className={classes.addcard_title}
          >
            {this.props.cards.length > 0 ? 'Add another card' : 'Add a card'}
          </p>
        </React.Fragment>
      );
    }
      ...
      <Draggable draggableId={this.props.column.id} index={this.props.index}>
        {provided => (
          ...
          >
            <Backdrop
              provided={provided}
              clickedOutside={this.onTitleOutsideClicked}
            >
            ...
            <Backdrop clickedOutside={this.onAddCardOutsideClicked}>
              {this.cardtTitleWrapper()}
            </Backdrop>
          </div>
        )}
      </Draggable>
